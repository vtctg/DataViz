  ...
  dashboardBody(
    ...
    tabItems(
      ...
      tabItem(tabName = "stats",
              fluidRow(box(width=7, status="warning",plotlyOutput("OvMap"), 
                       ...
                       box(width=5, title=strong("Input",style="font-family:'Verdana'"),status="info",solidHeader = TRUE,
                           h4("The left graphs show some environmental statistics by countries."),
                           br(),
                           radioButtons("MaPcaT", label = "Select category:",
                                        choices = list("Population" = "population", 
                                                       "Electricity generation (kWh)" = "electricity",
                                                       "Coal consumption (tonnes oil equivalent)" = "coal",
                                                       "CO2 emission (kt, kilotonnes)" = "co2"),
                                        selected = "population"),
                           sliderInput("YeAR2", label = "Select year:",
                                       min=1990, max=2018, value=2018),
                           br(),
...
>>>>>>> 3cbcd0ab95c134a9da8758d16e644243da052fb8
              ),
              
              tabItem(tabName = "handkpca",
                      fluidRow(box(width=5, title=strong("Input",style="font-family:'Verdana'"),status="info",solidHeader = TRUE,
                                   h4("The right graphs show the relationship between two of the below selected variables in 2018."),
                                   br(),
                                   radioButtons("V1", label = "Select x-axis:",
                                                choices = list("Population" = "population", 
                                                               "Electricity generation (kWh)" = "electricity",
                                                               "Coal consumption (tonnes oil equivalent)" = "coal",
                                                               "CO2 emission (kt, kilotonnes)" = "co2"),
                                                selected = "population"),
                                   radioButtons("V2", label = "Select y-axis:",
                                                choices = list("Population" = "population", 
                                                               "Electricity generation (kWh)" = "electricity",
                                                               "Coal consumption (tonnes oil equivalent)" = "coal",
                                                               "CO2 emission (kt, kilotonnes)" = "co2"),
  ...
  server <- function(input, output) {
    ...
    output$Mgy = renderPlotly(ggplotly(ggplot() + geom_polygon(data=filter(CountrYrOz, Year == input$YeArrrrr), mapping = aes(long,lat,group=group, fill=Consumption, color=Country), size=0) +theme_void() +scale_fill_gradient(  low = "#99FFFF", na.value="#000000",high = "#FF0000")+theme(panel.background = element_rect(fill = '#FFFFFF', colour = 'blue'))))
    ...
    output$OvMap = renderPlotly(ggplotly(ggplot() + geom_polygon(data=filter(mapping, Year == input$YeAR2), mapping = aes_string("long","lat",group="group", fill=input$MaPcaT, color="Country"), size=0) +theme_void() +scale_fill_gradient(  low = "#99FFFF", na.value="#000000",high = "#FF0000")))
    ...
    hulls <- plyr::ddply(ind.coord, "cluster", find_hull)
